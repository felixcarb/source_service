...
class TestFTPSSourceOperations(unittest.TestCase):
    """Pruebas unitarias para move_document y delete_document de FTPSSource."""

    def setUp(self):
        self.fetcher = FTPSSource()
        self.config = {
            'host': 'testhost',
            'username': 'testuser',
            'password': 'testpass',
    ...
    @patch('source_service.fetchers.ftps_fetcher.FTPSSource._connect')
    ...
    def test_move_document_error(self, mock_connect):
        ...
        mock_ftp = Mock()
        mock_ftp.rename.side_effect = Exception("FTPS rename error")
        mock_connect.return_value = mock_ftp
    ...
    def test_delete_document_error(self, mock_connect):
        ...
        mock_ftp = Mock()
        mock_ftp.delete.side_effect = Exception("FTPS delete error")
        mock_connect.return_value = mock_ftp
